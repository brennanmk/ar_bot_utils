...
class TrainARBot:
    def __init__(self, args) -> None:
        ...
        config = (
            ...
            .training(
                ...
                model={"fcnet_hiddens": args.network},
                train_batch_size=args.batch_size,
            )
            .evaluation(
                evaluation_interval=self.evaluation_interval,
                evaluation_duration=args.evaluation_duration,
                evaluation_config={"expore": False},
            )
        )
        ...
        for expirement in range(args.num_expirements):
            ...
            requests.post(
                "https://ntfy.bmillerklugman.me/phone",
    ...
    def eval_callback(self, trial_id: str, result: dict) -> bool:
        if "evaluation" in result:
            evaluation_mean_reward = result["evaluation"]["sampler_results"][
                "episode_reward_mean"
            ]
            return (
                evaluation_mean_reward >= self.stop_reward
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpu", type=int, default=0, help="number of gpu to use during training"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    parser.add_argument(
        "--discount", type=float, default=0.99, help="discount factor for training"
    )
    parser.add_argument(
        "--clip_param", type=float, default=0.2, help="clip for training"
    )
    parser.add_argument(
        "--evaluation_duration",
        type=int,
        default=20,
        help="evaluation duration for training",
    )
    parser.add_argument(
        "--evaluation_interval",
        type=int,
    ...
    parser.add_argument(
        "--network", nargs="+", default=[32, 32], help="Size of training network"
    )
    parser.add_argument(
        "--obstacles",
        type=int,
        default=0,
        help="number of obstacles to use during training",
    )
    parser.add_argument(
        "--timesteps",
        type=int,
        default=400,
        help="number of timesteps per episode to use during training",
    )
    parser.add_argument(
        "--stop_return",
        type=float,
        default="-4.0",
        help="evaluation reward to stop training",
    )
    parser.add_argument(
        "--iterations",
        type=int,
        default=3000,
        help="number of iterations total to use during training",
    )
    parser.add_argument(
        "--num_expirements",
        type=int,
        default=1,
        help="number of expirements to collect",
    )
    parser.add_argument(
        "--expirement_name",
        type=str,
        default=None,
        help="name of expirement to save csv to",
    )
    parser.add_argument(
        "--batch_size", type=int, default=4000, help="size of train batch"
    )
